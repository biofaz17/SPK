...
enum Screen {
  ...
  LOGIC_GAME, PARENTS, CHECKOUT, PAYMENT_SUCCESS
}
...
export default function App() {
  ...
  // Inicialização: Verifica sessão na "nuvem"
  useEffect(() => {
    const init = async () => {
      try {
  ...
  // Sincronização Automática: Toda vez que o 'user' muda, envia para o DataService (APENAS SE NÃO FOR VISITANTE)
  useEffect(() => {
  ...
  return (
    <div className="antialiased text-slate-800 font-sans selection:bg-indigo-100">
      ...
      {user && ![Screen.GAME, Screen.MATH_GAME, Screen.MEMORY_GAME, Screen.RHYTHM_GAME].includes(screen) && (
