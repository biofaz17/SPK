...
export const GeometryGameScreen: React.FC<GeometryGameScreenProps> = ({ onBack }) => {
  ...
  const nextShape = () => {
      ...
      const wrongOptions = SHAPES.filter(s => s.name !== shape.name)
  ...
  return (
    <div className="min-h-screen bg-yellow-50 text-slate-800 flex flex-col font-sans">
      <div className="p-4 flex justify-between items-center z-10 bg-white/80 backdrop-blur-md border-b border-yellow-200">
        ...
        <div className="bg-yellow-100 px-3 py-1 rounded-full text-yellow-800 font-bold text-sm">
           Acertos: {score}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 z-10 gap-8">
          <motion.div 
            className={`w-64 h-64 bg-white rounded-3xl shadow-xl flex items-center justify-center p-8 border-4 border-yellow-300 ${shake ? 'animate-shake border-red-400' : ''}`}
          >
              <motion.svg 
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full max-w-lg">
              {options.map((opt, i) => (
                  <button 
                     ...
                     className="bg-white border-b-4 border-yellow-200 hover:border-yellow-400 hover:bg-yellow-50 active:border-b-0 active:translate-y-1 text-slate-700 font-bold py-4 rounded-xl shadow-sm transition-all text-lg"
                  >
                      {opt}
                  </button>
              ))}
