...
export const MathGameScreen: React.FC<MathGameScreenProps> = ({ onBack }) => {
  ...
  const generateQuestion = () => {
    ...
    const ans = isPlus ? num1 + num2 : (num1 > num2 ? num1 - num2 : num1 + num2); // Evita negativos por enquanto
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white flex flex-col relative overflow-hidden font-sans">
       ...
       <div className="p-4 flex justify-between items-center z-10 bg-slate-800/50 backdrop-blur-md border-b border-slate-700">
          <button onClick={onBack} className="bg-slate-700 p-2 rounded-full hover:bg-slate-600">
             <ArrowLeft />
          </button>
       ...
       <div className="flex-1 flex flex-col items-center justify-center p-6 z-10">
          ...
          <div className="flex w-full max-w-2xl justify-between items-end mb-12 px-4 relative">
              ...
              <AnimatePresence>
                 {gameState === 'correct' && (
                    <motion.div 
                        initial={{ left: 50, bottom: 50, scale: 0.5, opacity: 1 }}
                        animate={{ left: "80%", bottom: "50%", scale: 2, opacity: 0 }}
                        className="absolute w-20 h-20 bg-emerald-400 rounded-full blur-xl z-20"
                    />
                 )}
              </AnimatePresence>
              ...
              <div className="flex flex-col items-center gap-2">
                  <div className="flex gap-1 mb-2">
                     {[...Array(3)].map((_, i) => (
                         <div key={i} className={`w-3 h-3 rounded-full ${i < heroHp ? 'bg-red-500' : 'bg-slate-700'}`} />
                     ))}
                  </div>
                  <motion.div 
                    animate={gameState === 'correct' ? { y: [0, -20, 0] } : {}}
                    className="text-6xl md:text-8xl filter drop-shadow-2xl"
                  >
                     🧙‍♂️
                  </motion.div>
              ...
              <div className="flex flex-col items-center gap-2">
                  <div className="flex gap-1 mb-2">
                     {[...Array(3)].map((_, i) => (
                         <div key={i} className={`w-3 h-3 rounded-full ${i < monsterHp ? 'bg-purple-500' : 'bg-slate-700'}`} />
                     ))}
                  </div>
                  <motion.div 
                    animate={gameState === 'correct' ? { x: [0, 10, -10, 0], opacity: [1, 0.5, 1] } : {}}
                    className="text-6xl md:text-8xl filter drop-shadow-2xl grayscale-[0.2]"
                  >
                     {level === 1 ? '👾' : (level === 2 ? '👹' : '🐉')}
                  </motion.div>
          ...
          {gameState === 'victory' ? (
             ...
             <div className="w-full max-w-md bg-slate-800/80 backdrop-blur-lg p-8 rounded-[2rem] border-2 border-slate-700 shadow-2xl">
                 <div className="text-center mb-8">
                    ...
                    <div className="text-5xl font-black text-white bg-slate-900/50 p-4 rounded-xl border border-slate-600 inline-block min-w-[200px]">
                        {question.q} = ?
                    </div>
                 </div>
                 ...
                 <div className="grid grid-cols-3 gap-4">
                    {question.options.map((opt, i) => (
                        <button 
                           ...
                           className={`
                             py-4 rounded-xl text-2xl font-black transition-all transform active:scale-95
                             ${gameState === 'wrong' ? 'bg-red-900/50 border-red-500' : 'bg-slate-700 hover:bg-emerald-600 border-b-4 border-slate-900 hover:border-emerald-800'}
                           `}
                        >
                           {opt}
                        </button>
