...
export const Robot: React.FC<RobotProps> = ({ 
  ...
  // Cálculo de rotação e direção visual
  if (!isSad) {
    switch (direction) {
  ...
  // Configuração de mola para movimento ultra fluido
  // Fix: Explicitly define the transition type as 'spring' using 'as const' to avoid TypeScript inference issues
  const springTransition = {
    type: "spring" as const,
    stiffness: 120, // Menos rígido para ser mais suave
    damping: 20,    // Amortecimento equilibrado para evitar oscilação excessiva
    mass: 1,
    restDelta: 0.001
  };
